...
export function StudioAudioPanel({
  ...
  function upload() {
    ...
    request.addEventListener("load", () => {
      ...
      setBusy(false);
      if (request.status >= 200 && request.status < 300 && response?.timeline) {
        onState(response.timeline);
        setFile(null);
        setName("");
        setConfirmed(false);
        setProgress(0);
  ...
  async function remove(asset: StudioMediaAssetDto) {
    setError(null);
    try {
      ...
      onState({
        ...(await refreshTimeline(studioProjectId)),
      });
    } catch (reason) {
  ...
  return (
    <div className="mt-5 rounded-xl border border-white/8 bg-black/15 p-4">
      ...
      <p className="mt-2 text-xs leading-5 text-slate-600">
        Upload only audio you recorded, own, or have permission to use. Files
        stream into this local app and are never sent to a service.
      </p>
